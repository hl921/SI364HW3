...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/haeleeHW3"
## Provided:
...
def index():
    ...
    ## And redirect to the list of all tweets
        user_id = u.id
        t = Tweet.query.filter_by(text=text, user_id=user_id).first()
        if t:
...
def see_all_tweets():
    ...
    tweets_and_users = []
    for twt in tweets:
        user = User.query.filter_by(id=twt.user_id).all()
        tup = (twt.text, user[0].username)
        tweets_and_users.append(tup)
    return render_template('all_tweets.html', all_tweets = tweets_and_users)
...
def get_longest_tweet():
    ...
    for twt in tweets:
        user = User.query.filter_by(id=twt.user_id).all()
        text = str(twt.text)
        # no_white = text.replace(' ','')
        tup = (text, user[0].username, user[0].display_name)
        list_of_tweets.append(tup)

    alph_sort = sorted(list_of_tweets, key= lambda x:x[0])
    sorted_tweets = sorted(alph_sort, key=lambda x: len(x[0].replace(' ','')), reverse=True)
    return render_template('longest_tweet.html', tweets = sorted_tweets)
